<div class="modal fade" id="updateTypeModal" tabindex="-1" role="dialog" aria-labelledby="updateTypeModalLabel"
    aria-hidden="true">

    <div class="modal-dialog modal-md" role="document">
        <form class="form" action="" method="POST" enctype="multipart/form-data" id='update_type_form'>
            @csrf
            @method('PUT')
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="updateTypeModalLabel">{!! __('sponsership.update_type') !!}
                    </h5>
...
@push('scripts')
    <script type="text/javascript">
        ...
        // cancel
        $('body').on('click', '#cancel_type_btn_edit', function(e) {
            $('#updateTypeModal').modal('hide');
            $('#update_type_form')[0].reset();
            resetEditForm();
        });

        // hide
        $('#updateTypeModal').on('hidden.bs.modal', function(e) {
            $('#updateTypeModal').modal('hide');
            $('#update_type_form')[0].reset();
            resetEditForm();
        });
        ...
        // update
        $('#update_type_form').on('submit', function(e) {
            e.preventDefault();
            // reset
            resetEditForm();

            // paramters
            var type_id = $('#id_edit').val();
            //var currentPage = $('#yajra-datatable').DataTable().page();
            var data = new FormData(this);
            var type = $(this).attr('method');
            var url = "{!! route('dashboard.sponsershipTypes.update', 'id') !!}".replace('id', type_id);
